...
{
  if (nodo.left().null() && nodo.right().null()) { // No tiene hijos
...
typename BST<Key, T>::iterator BST<Key, T>::erase(const Key & clave)
{
  ...
  bool encontrado = encontrar(e, nodo, nodo_padre);
  typename BST<Key, T>::iterator it;
...
typename BST<Key, T>::iterator& BST<Key, T>::iterator::operator++()
{
  ...
  if (!elnodo.right().null()) { // Tiene hijo derecha
      elnodo = elnodo.right();
      while (!elnodo.left().null())
        elnodo = elnodo.left();
  }
  else {
      while (!elnodo.parent().null() && elnodo.parent().right() == elnodo)
...
template <class Key, class T>
inline
typename BST<Key, T>::iterator BST<Key, T>::iterator::operator++(int)
{
  typename BST<Key, T>::iterator aux(*this);
  this->operator++();
