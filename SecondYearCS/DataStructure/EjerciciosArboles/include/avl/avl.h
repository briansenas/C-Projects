 ...
 *  @brief TDA. AVL
 *
 * TDA AVL::AVL, insert, erase, find, size, empty
 * clear, begin, end, ~AVL
 *
 * El TDA AVL representa objetos que abstraen el concepto de Arbol
...
public:
  ...
  */
  ...
  /**
      @brief Inserta o actualiza una entrada en el AVL.
  ...
  */
  ...
  /**
     @brief Buscar una clave en el AVL

     @param clave: Clave a buscar en el AVL.
     @return  iterador a la posici�n del AVL con la clave
              'clave', si est� en el AVL. end, en otro caso.
  ...
  */
  ...
  /**
     @brief Eliminar una entrada en el AVL.
...
private:
  struct etiqueta_nodo  {
      Key clave;
      T etiqueta;
      int h;
  };
  typedef etiqueta_nodo entry;
  void ajustar_avl(const typename bintree<etiqueta_nodo>::node &n);
  void rotar_izquierda(const typename bintree<etiqueta_nodo>::node &n);
  void rotar_derecha(const typename bintree<etiqueta_nodo>::node &n);
  int altura(const typename bintree<etiqueta_nodo>::node &n);
...
class iterator {
  public:
    ...
    */
    ...
    /**
       ...
       @pre El AVL asociado NO ha sido modificado despu�s
       de la creaci�n del iterador receptor.
       @pre El iterador receptor NO est� al final del recorrido:
       (receptor) != end()

       @return  Posici�n actual en el recorrido creciente del AVL
                asociado.
  ...
  private:
    ...
    typename bintree<etiqueta_nodo>::node elnodo;
  };
  ...
  {
      cout << "Árbol";
      std::queue<typename bintree<etiqueta_nodo>::node> Q;
      Q.push(arbolb.root());
      while (!Q.empty()) {
          typename bintree<etiqueta_nodo>::node n = Q.front();
          Q.pop();
          ...
          if (n.null())
              ...
              Q.push(n.left());
              Q.push(n.right());
              Q.push(typename bintree<etiqueta_nodo>::node());
          }
      }
      cout << endl;
  }
 ...
 private:
  ...
  /**
   *  @brief AVL<Key, T>::entry
   *
   * Es el tipo de dato base contenido en AVL
   */
  ...
  void borrar_nodo(typename bintree<typename AVL<Key, T>::entry>::node nodo,
		 typename bintree<typename AVL<Key, T>::entry>::node nodo_padre);

  void preorden_con_altura(const typename bintree<etiqueta_nodo>::node & n) const
  {
      if (!n.null()) {
          cout << "Etiqueta:" << (*n).clave << " altura:" << (*n).h << endl;
          preorden_con_altura(n.left());
          preorden_con_altura(n.right());
      }
  }
