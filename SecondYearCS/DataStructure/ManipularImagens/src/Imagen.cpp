...
Imagen::Imagen(const Imagen& otro){
    ...
    this->valores = new byte*[this->filas];
    for(int i=0;i<this->filas;i++){
        this->valores[i] = new byte[this->columnas];
...
void Imagen::asigna_pixel(int nf, int nc, byte valor){
...
byte Imagen::valor_pixel(int nf,int nc){
    byte result = -1;
    if(nf>0 && nf<this->filas && nc>0 && nc<this->columnas)
...
void Imagen::LeerImagenPGM(){
    if (this->tipo == IMG_PGM){
        byte** resultado = new byte*[this->filas];
        for (int i = 0; i < this->filas; i++) {
            resultado[i] = new byte[this->columnas];
...
void Imagen::Enmarcar(int width,int R,int G,int B){
    ...
    byte** resultado = new byte*[sizeRow];
    for(int i=0;i<sizeRow;i++){
        resultado[i] = new byte[sizeCol];
...
bool Imagen::ConvertTo_Gray(){
    ...
    if(this->tipo==IMG_PPM){
        ...
        byte** resultado = new byte*[this->filas];
        for(int i=0;i<this->filas;i++){
            resultado[i] = new byte[this->columnas/3];
...
void Imagen::ReducirPorN(int nf, int nc){
    ...
    this->HardReset();
    int sz = nf*nc;
    this->valores = new byte*[sizeRow];
    for(int i=0;i<sizeRow;i++){
        this->valores[i] = new byte[sizeCol];
        if(i>checkRow && modf)
    ...
    //Además en la conversión de int a unsigned char, dejamos 3 bytes perdidos....
    for(int i=0;i<sizeRow;i++)
        delete [] resultado[i];
    delete [] resultado;
    ...
    for(int x=0;x<offset;x++) {
        for(int i=0;i<this->filas;i++){
            for(int j=x;j<this->columnas;j+=offset){
                ...
                this->valores[i][j] = (byte)((int)valor);
            }
        }
    }
}
...
void Imagen::Decodificar(string& msg){
    ...
    for(int i=0;i<this->filas;i++){
        for(int j=0;j<this->columnas;j++){
            b = this->valores[i][j] & 1; // Cogemos un byte de todo a 0 excepto el ultimo
